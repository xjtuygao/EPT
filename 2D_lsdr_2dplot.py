...
for loop in range(nloop):
  ...
  #             			  density=2, arrowstyle='->', arrowsize=1.0)
		...
		ax = plt.axes()
		contourf = ax.pcolormesh(X, Y, mesh_DR, cmap='viridis', norm=Normalize(0.8, 1.2))
		fig.colorbar(contourf)
		ax.set_xlim(LOW,HIGH)
		ax.set_ylim(LOW,HIGH)
		plt.savefig('./vf/%s/2d_dr_%s_%s_%s_gp.pdf' % (data_target, data_target, div, loop), bbox_inches='tight')
		plt.close()
