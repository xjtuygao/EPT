...
s = toy_data_gen(data_target, batch_size=nplot)
x = s[:, 0]
y = s[:, 1]
...
plt.xticks([LOW, HIGH])
plt.yticks([LOW, HIGH])
plt.savefig("./figure/%s/kde_%s_real_targ-dd.pdf" % (data_target, data_target))
plt.close()
...
plt.xticks([LOW, HIGH])
plt.yticks([LOW, HIGH])
plt.savefig("./figure/%s/scatter_%s_real_targ-dd.pdf" % (data_target, data_target))
plt.close()
...
# plt.xticks([LOW, HIGH])
# plt.yticks([LOW, HIGH])
# plt.savefig("./figure/%s/hist_%s_real_targ-dd.pdf" % (data_target, data_target))
# plt.close()
...
plt.xticks([LOW, HIGH])
plt.yticks([LOW, HIGH])
plt.savefig("./figure/%s/kde_%s_real_ref-dd.pdf" % (data_target, data_target))
plt.close()
...
plt.xticks([LOW, HIGH])
plt.yticks([LOW, HIGH])
plt.savefig("./figure/%s/scatter_%s_real_ref-dd.pdf" % (data_target, data_target))
plt.close()
...
for loop in range(nloop):
	...
	if (loop+1) % print_period == 0 or loop == 0:
		print('loop(%s/%s)%d: %.4f | %.4f' 
			  % (div, data_target, loop, LOSS_DD[-1], Grad_norm[-1]))
		...
		plt.xlim(LOW,HIGH)
		plt.ylim(LOW,HIGH)
		plt.savefig('./vf/%s/gf_%s_%s_%s-dd.pdf' % (data_target, data_target, div, loop), bbox_inches='tight')
		plt.close()

		# surface
		fig = plt.figure(figsize=(10, 8))
		ax = plt.axes()
		contourf = ax.pcolormesh(X, Y, mesh_DD, cmap='viridis', norm=Normalize(-0.2, 0.2))
		fig.colorbar(contourf)
		ax.set_xlim(LOW,HIGH)
		ax.set_ylim(LOW,HIGH)
		plt.savefig('./vf/%s/2d_dd_%s_%s_%s-dd.pdf' % (data_target, data_target, div, loop), bbox_inches='tight')
		plt.close()
	...
	if (loop+1) % plot_period == 0 or loop == 0:
		...
		plt.xticks([LOW, HIGH])
		plt.yticks([LOW, HIGH])
		plt.savefig("./figure/%s/kde_%s_%s_%d-dd.pdf" % (data_target, data_target, div, loop))
		plt.close()
		...
		plt.xticks([LOW, HIGH])
		plt.yticks([LOW, HIGH])
		plt.savefig("./figure/%s/scatter_%s_%s_%d-dd.pdf" % (data_target, data_target, div, loop))
		plt.close()
		...
		plt.xticks([LOW, HIGH])
		plt.yticks([LOW, HIGH])
		plt.savefig("./vf/%s/visu_map_%s_%s_%d-dd.pdf" % (data_target, data_target, div, loop))
		plt.close()
		...
		plt.ylabel('Loss_dd')
		plt.legend()
		fig.savefig('./loss/Loss_dd-%s-dd.png' % data_target)
		plt.close()
		...
		plt.legend()
		fig.savefig('./loss/Grad_norm-%s-dd.png' % data_target)
		plt.close()
		...
		dataframe.to_csv('saved_loss-%s-dd.csv' % data_target, sep=',')
